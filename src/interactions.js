// ---- 交互：画布内鼠标/键盘事件绑定、自由传送带模式状态机、工具栏拖拽生成新设备 ----
import { GRID_SIZE, HINT_NORMAL, HINT_BELT } from './constants.js';
import { state, canvas, toolbar, crusherIcon, ghostIcon, hintEl } from './state.js';
import { screenToWorld, worldToCell } from './coords.js';
import {
  hitTestDevice, findPortAt, effectiveGridPos, getDevicePorts,
  isInputPortUsed, isOutputPortUsed, flowDirOf, spawnTemplate
} from './devices.js';
import {
  buildBlockedSet, buildBeltOccupancy, aStarOrthogonal, removeSelfOverlap,
  computePath, recomputeAllConnections, hitTestConnection, hitTestWaypoint,
  waypointInsertIndex, splitConnectionAtCell, cellOrientationsOf,
  pickBestPort, pickNearestPortByDistance, resolveConnEndpoint
} from './pathfinding.js';
...
export function updateHintText() {
  hintEl.textContent = state.freeBeltMode ? HINT_BELT : HINT_NORMAL;
...
function bindCanvasMouseEvents() {
  canvas.addEventListener('mousedown', (e) => {
    ...
    // 普通模式下，按住已连接的输入口(传送带末端箭头)可以把这段线的末端拖到
    ...
    if (wpHit) {
      pushHistory();
      const wpConn = state.connections.find(c => c.id === wpHit.connId);
      const originalCell = wpConn && wpConn.waypoints[wpHit.index] ? { ...wpConn.waypoints[wpHit.index] } : null;
      state.draggingWaypoint = { connId: wpHit.connId, index: wpHit.index, originalCell };
      state.selectedId = null;
      state.selectedConnectionId = wpHit.connId;
      draw();
      return;
    }
    ...
    if (connHit) {
      state.selectedConnectionId = connHit.conn.id;
      state.selectedId = null;
      // 先记录待定状态：真正拖动后才在该线段处插入新途经点，单纯点击仍只是选中
      state.pendingWaypointCreate = { connId: connHit.conn.id, segmentIndex: connHit.segmentIndex, downX: e.clientX, downY: e.clientY };
      draw();
      return;
    }
  ...
  window.addEventListener('mouseup', (e) => {
    ...
    if (state.draggingDeviceId !== null) {
      ...
      if (dev) {
        ...
        // 这里只管"别的连线被顺手拖坏"这一种情况。
        if (state.draggingDeviceBeforeSnapshot && brokeExistingValidConnection(state.draggingDeviceBeforeSnapshot)) {
          revertLastHistoryStep();
        }
      }
    }
...
function bindKeyboardEvents() {
  window.addEventListener('keydown', (e) => {
    ...
    if (state.selectedConnectionId !== null) {
      ...
      e.preventDefault();
      pushHistory();
      // 删除设备只移除设备本身：与它相连的传送带保留下来，端点改为设备原先
      // 所在端口紧邻的那格自由网格端点，成为一段悬空的传送带，仍可在普通模式下
      // 选中/删除，或拖拽途经点调整。
      const dev = state.devices.find(d => d.id === state.selectedId);
      if (dev) {
        const pos = effectiveGridPos(dev);
...
function bindToolbarSpawnEvents() {
  ...
  window.addEventListener('mouseup', (e) => {
    ...
    if (state.spawnPreview && !isPointInToolbar(e.clientX, e.clientY)) {
      pushHistory();
      const newDevice = {
        id: state.nextId++,
