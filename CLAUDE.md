...
- **文件结构**（`project_status.md` 的"重构建议"已落地）：
  ...
  - `src/devices.js` —— 设备数据模型、端口计算（`getDevicePorts`/`edgePorts`/`nodeDevicePorts`/`facilityDevicePorts`）、碰撞检测（`computeCollidingIds`）、电力覆盖判定（`computeUnpoweredIds`/`getPowerRangeRect`，纯派生计算，和碰撞检测同一套"每次从 state.devices 现算、不进撤销栈"的先例）；`SPAWN_TEMPLATES` 由两部分拼接而成：`src/data/facilities.js` 的 `FACILITIES` 在模块加载时 `flatMap` 生成的真实基建设备模板，加上文件内手写的 `NODE_TEMPLATES`（汇流器/分流器/管道汇流器/管道分流器四种 1×1 节点，归在独立的"物流"分类标签页，和游戏里的同名分类对应——这四种不是真实游戏建筑数据，不应该也塞进 `facilities.js`）。落地新设备的代码内联在 `interactions.js` 的工具栏 mouseup 处理里，不是独立的 `spawnTemplate` 函数。
...
- **整体是白色基调的工业风**：画布背景、toolbar、hint 胶囊都是白/浅灰，不是早期版本的深色 UI。颜色沿用现有调色板，不要新增无关色系：
  ...
  - 警告/无效态统一用红色系，且**半透明**，呼应设备重叠警示的 `rgba(255, 0, 0, 0.45)` / `#ff1744`（传送带无效态用的是 `rgba(255, 23, 68, 0.35/0.55)`，同一色相不同透明度）。新增任何"这个操作不合法/位置不可用"的视觉反馈，复用这个红色语义，不要发明新颜色。**例外**：设备"断电"（`needsPower` 为真但不在任何供电范围内）不是"操作不合法"，是设备的运行状态警告，且经常需要和重叠警示同屏出现、必须能区分开，因此单独用橙色 `POWER_WARNING_COLOR = '#ff9100'`（`devices.js` 的 `computeUnpoweredIds()` + `render.js` 的断电徽标）——这是目前唯一被批准偏离"警告统一红色"规则的场景，不要因为这个先例就给其它"操作不合法"类警告也另起新颜色。
