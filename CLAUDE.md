...
- **文件结构**（`project_status.md` 的"重构建议"已落地）：
  ...
  - `src/devices.js` —— 设备数据模型、端口计算（`getDevicePorts`/`edgePorts`/`nodeDevicePorts`/`facilityDevicePorts`）、碰撞检测；`SPAWN_TEMPLATES` 由 `src/data/facilities.js` 的 `FACILITIES` 在模块加载时 `flatMap` 生成（不是手写，也不是独立的 `spawnTemplate` 函数——落地新设备的代码内联在 `interactions.js` 的工具栏 mouseup 处理里）。
