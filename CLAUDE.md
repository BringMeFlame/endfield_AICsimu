...
- **文件结构**（`project_status.md` 的"重构建议"已落地）：
  ...
  - `src/state.js` —— DOM 元素引用（`canvas`/`ctx`/`toolbar`/`toolbarTabs`/`toolbarIcons`/`hintEl`/`powerSummaryEl`，各自独立 `export`，**不在** `state` 对象里）+ 一个导出的可变 `state` 对象，装下原来所有顶层 `let`（`devices`、`connections`、`selectedId`、`freeBeltMode`、`activeToolbarCategory` 等，见下方"全局变量习惯"）。
