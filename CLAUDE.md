...
- **文件结构**（`project_status.md` 的"重构建议"已落地）：
  ...
  - `src/recipeSlots.js` —— 配方槽位系统（预览阶段，还没最终定稿，见下方"配方+物品槽位系统"一节）的匹配/写入逻辑：`getSlotPanelKind()` 判定一个设备走哪种槽位面板（`'recipe'`＝配方自动匹配、`'port'`＝端口自由选物品、`null`＝没有槽位面板）、`buildInitialSlotState()` 落地新设备时生成初始槽位数据、`getPortSlotDescriptors()`（'port' 模式的槽位描述符，默认一个端口一个槽位，热能池是唯一例外——只给一个不对应任何真实端口 id 的虚拟槽位，因为它虽然有 2 个传送带口，但同一时刻只烧一种燃料）、`computeSlotCandidates()`（原料/产物槽位共用同一个双向收紧算法，见文件内 `isRecipeViable` 的注释）、`normalizeSlotValues()`（每次槽位变化后跑 `pruneInvalidSelections` 清掉不再合法的旧选择，**不会**自动帮用户填任何槽位——试过"收紧到唯一解自动填入"，用户反馈手感奇怪，已经去掉）、`getRelevantItemIds`（抽屉"本设备相关"标签页用）、`setSlotValue`/`clearAllSlots`（写入前照例 `pushHistory()`，写完跑一遍 `normalizeSlotValues`）。同时依赖 `devices`/`data/recipes`/`data/items` 三份数据，单独开一个模块而不是塞进 `devices.js`。
...
- **已实现**：
  - `'recipe'` 模式——原料/产物哪个先选都行，双向对称收紧（`recipeSlots.js` 的 `isRecipeViable`：已选原料 ⊆ 配方需求 且 已选产物 ⊆ 配方产出，两个方向同一个判定，不像早期版本那样只能"先选原料"）。`computeSlotCandidates(facilityId, slotValues, group)` 是原料槽位/产物槽位共用的唯一一个候选计算函数，不再分 `computeInputCandidates`/`computeOutputCandidates` 两个函数。**槽位不会自动帮用户填**——`normalizeSlotValues()` 每次槽位变化后只跑 `pruneInvalidSelections` 清掉不再合法的旧选择，试过"收紧到唯一解自动填入"那版，用户反馈手感奇怪，已经去掉，不要再加回来。
  - `'port'` 模式——槽位就是端口本身，自由选同 portType 物品，不做收紧。除了暗管入口/出口、多口暗管、仓库存货口/取货口、储液罐/储气罐之外，气体散布机（任意气体）、废水处理机（任意流体）、热能池（任意固体，但只有源矿/原木/各类电池是真燃料，放别的东西会触发 `devices.js` 的 `HEAT_POOL_FUEL_ITEM_IDS` 警告）也是这个模式——这三个设备没有真实配方数据，"随便接什么"本来就是它们的定位，槽位数直接取自 `recipeSlots.js` 的 `getPortSlotDescriptors()`，默认等于 `ports` 数量，**热能池是唯一例外**——虽然有 2 个传送带口，但同一时刻只烧一种燃料，只给 1 个不对应任何真实端口 id 的虚拟槽位（key 是硬编码的 `'fuel'`），不能假设 `'port'` 模式的槽位数总是等于端口数。**协议核心/次级核心不在 `PORT_ITEM_FACILITY_IDS` 里**——核心端口一多，槽位面板显示很乱，用户已确认暂时不需要，右键核心走浏览器默认菜单，不弹任何面板。
  ...
  - **惰气/酸气环境覆盖判定**——气体散布机（`dev_气体散布机`）接入惰气（`item_gas_inert`）/酸气（`item_gas_acid`）时，以自身占地中心产生一个 13×13 的环境覆盖范围（`devices.js` 的 `getGasEnvRangeRect`/`computeGasEnvRangeRects`，和供电覆盖范围 `getPowerRangeRect` 同构，只是按气体种类分惰气/酸气两组矩形），常驻显示在画布上（`render.js` 的 `drawGasEnvRanges`，惰气蓝色/酸气橙色描边区分，不受 `H` 键开关控制，和默认隐藏的供电范围叠层不同）。`recipeSlots.js` 的 `getRequiredGasEnvType()` 复用 `isRecipeViable` 判断设备当前槽位选择是否需要环境（同一条"宁可多提醒"原则：只要有一条可行配方需要环境就提示，不要求所有可行配方都需要）；`devices.js` 的 `computeGasEnvWarnings()` 做"完全覆盖"判定——和供电"部分覆盖即通电"不同，要求设备整个占地都落在同类型范围内，允许多台同类型散布机的范围拼接覆盖；未覆盖时复用"未通电"同一套警告图标+悬浮浮窗框架，文案"需求惰气/酸气环境"。接入速率（游戏内"至少 30 单位/min"）本项目明确不做流速判定，和催化剂/热能池槽位同一条"只看有没有接对气体种类，不核实具体流速数值"的原则。**`detectGasEnvType()` 用 `startsWith` 而不是 `includes` 匹配 `environmentCondition` 开头的"需求惰气/酸气环境"文案**——这是修过的真实 bug：气体反应炉"气态灼铜生产"那条配方的说明文字里写着"与惰气环境类似"，早期用 `includes('惰气环境')` 判断会被这句说明文字误命中，导致明明是"需求酸气环境"开头的配方被误判成惰气，改用 `startsWith` 精确匹配开头后修复。
- **明确还没做**（不要假设这些已经存在）：
