...
- **文件结构**（`project_status.md` 的"重构建议"已落地）：
  ...
  - `src/recipeSlots.js` —— 配方槽位系统（预览阶段，还没最终定稿，见下方"配方+物品槽位系统"一节）的匹配/写入逻辑：`getSlotPanelKind()` 判定一个设备走哪种槽位面板（`'recipe'`＝配方自动匹配、`'port'`＝端口自由选物品、`null`＝没有槽位面板）、`buildInitialSlotState()` 落地新设备时生成初始槽位数据、`computeInputCandidates`/`computeOutputCandidates`（配方收紧算法的两个方向，见文件内注释里 `isRecipeSatisfied` vs `isRecipeViable` 的子集方向区别）、`getRelevantItemIds`（抽屉"本设备相关"标签页用）、`setSlotValue`/`clearAllSlots`（写入前照例 `pushHistory()`）。同时依赖 `devices`/`data/recipes`/`data/items` 三份数据，单独开一个模块而不是塞进 `devices.js`。
...
右键一个有槽位数据的设备弹出居中浮层槽位面板，点槽位从屏幕右侧滑出物品选择抽屉，方形槽位=固体、圆形槽位=流体（和游戏一致）。数据来自 `reference/items_recipes_template.xlsx`（人工核对进度见该文件/`reference/README.md`），转成 `src/data/items.js`/`src/data/recipes.js` + `facilities.js` 的 `slots` 字段接入。这一节记的是当前**已经写代码实现**的部分和**明确还没做**的部分，避免后续改动时误以为某个能力已经存在：

- **已实现**：`'recipe'` 模式（配方自动匹配收紧，`recipeSlots.js` 的 `computeInputCandidates`/`computeOutputCandidates`）+ `'port'` 模式（暗管入口/出口等端口设备自由选物品，不做任何收紧）两条路径共用同一个右键入口，槽位数据挂在 `dev.slotValues`/`dev.portItems` 上随设备一起进 `cloneCanvasState()` 的深拷贝，Ctrl+Z/框选批量拖拽/Ctrl+V 复制不需要额外处理。抽屉的"清空全部"/单槽位"×"两种粒度的撤销机制、Esc 分层关闭（先关抽屉再关面板）都已接入。
- **明确还没做**（不要假设这些已经存在）：
