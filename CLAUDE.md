...
- **自我重叠去重分两层，不要合并**：
  ...
  - 但"不截断"不代表"允许自我重叠"：传送带物理上不能穿过自己。`computePath` 在拼接完路径后用 `hasSelfOverlap()` 做**只读检测**（不修改路径），一旦发现完整路径经过了同一个格子两次就整体判定为 `invalid`，复用"找不到路径"的红色警示渲染出来；再由拖拽途经点/端点重接的 `mouseup` 处理逻辑把这次改动撤销（新插入的途经点整体移除、移动已有途经点则还原回拖拽前的位置）。也就是说自我重叠不是靠寻路阶段"不让走"来避免的，而是靠"允许算出来、但不接受这个结果"这条事后校验规则（这是修过的真实 bug：之前完全没有这层检测，导致拖拽途经点能摆出一条自我重叠、视觉上"走回头路"的传送带，详见 `project_status.md` 已知 Bug 历史记录）。
