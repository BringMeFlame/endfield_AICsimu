...
- 端口"必须笔直接入"是默认硬规则，但当唯一能笔直进入端口的那一格恰好被邻近设备的 footprint 整体占死时（典型场景：两台设备只隔 1 格空位、又有 1~2 格 Y 轴错位），笔直路线物理上不存在——此时退化为允许贴边拐入（`aStarWithDockingFallback`，先严格寻路，失败后才允许从侧面拐进端口前一格），且仍会检查这一格没有被其它传送带占用，不会绕过"传送带不允许重叠"这条约束。
...
- 修复了一处真实的寻路 Bug：两台设备之间只留 1 格空位、又存在 1~2 格 Y 轴错位时，明明存在合理的 Z 形绕行方案，连线却直接判定为无法寻路（红线）。排查发现根因不在转弯代价/启发式，而在于"输入口必须笔直接入"这条硬规则——笔直进入端口所需的那一格，恰好整格都在邻居设备的 3×3 footprint 里，导致这条唯一的直行车道物理上不存在，A* 无论怎么绕都无法从正确方向踏上那一格。用穷举脚本直接调用项目里真实的 `computePath`/`aStarOrthogonal`（对两台设备的旋转组合、间距、Y 偏移、端口组合做了约 13.5 万种非重叠布局的扫描）验证过，此前算法在所有"确实存在合法路径"的布局下都能正确找到路径，唯独这一类"直行车道被邻居整体占死"的布局会被判定无解；这类布局并非算法缺陷，而是设计上还没有"贴边拐入"的退路。修复：新增 `aStarWithDockingFallback`，仍然优先按原规则严格寻路(笔直接入)，只有严格寻路失败时才退化为允许从侧面拐进端口前一格——最后贴边对接进设备的那半格线段仍然固定沿端口方向画，视觉上入口依旧笔直，只是引导它的路径拐了个弯；同时这一步仍然检查目标格没有被其它传送带占用（和现有"转弯必须发生在空闲格"的规则一致），避免这条兜底路径绕过"传送带不允许重叠"的约束。已用穷举扫描 + 专门构造的"对接格已被占用"场景验证：原本被卡死的布局现在能正确生成路径，且不会引入新的重叠。
