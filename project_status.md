...
- 目前暂无已知的功能性崩溃/渲染错误——上一轮排查并修复了途经点拖拽导致传送带"纹丝不动"的 bug（整体去重误吞并合法折返路径）以及自由传送带模式下点击已有汇流器/分流器节点被误判为点击传送带本身的问题；本轮修复了上一轮那次修复引入的回归——去掉整体截断后，`computePath` 对拼接后的多跳路径完全没有自我重叠检测，导致拖拽途经点/端点重接能摆出一条自我重叠、视觉上"走回头路"的传送带（同一段带子同时表示两个方向的物料流动），且被当成合法路径正常渲染成黄色，不会有任何红色警示。修复方式：新增只读检测 `hasSelfOverlap`，拼接后一旦发现自我重叠就整体判定为 `invalid`（红色警示渲染，不截断路径本身），并在途经点拖拽/端点重接的 `mouseup` 逻辑里补上"若结果无效则撤销这次改动"，使已提交的连线状态始终保持"不自我重叠"这一硬性约束，均已在浏览器中验证（含"确实会自我重叠的拖拽位置在松手后正确撤销"与"合法绕行路径正常提交"两种场景）。
